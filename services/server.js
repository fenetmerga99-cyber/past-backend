...
// cron-job.org (or any uptime pinger) hits the bare root URL with nothing
// after it — without this route Express 404s on "/", which cron-job.org
// reports as a failed run even though the ping still keeps Render awake.
app.get('/', (req, res) => res.json({ ok: true, service: 'ai-solver-backend' }));
